...
from fastapi_zero.schemas import Message, UserPublic, UserSchema, UsersList
...
def update_user(
    ...
    if current_user.id != user_id:
        raise HTTPException(
            status_code=HTTPStatus.FORBIDDEN, detail='Not enough permitions'
        )
...
def delete_user(user_id: int, session: T_Session, current_user: T_CurrentUser):
    if current_user.id != user_id:
        raise HTTPException(
            status_code=HTTPStatus.BAD_REQUEST, detail='Not enough permitions'
        )
